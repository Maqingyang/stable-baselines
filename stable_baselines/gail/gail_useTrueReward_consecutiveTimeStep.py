...
from stable_baselines.common.runners_consecutive import traj_segment_generator
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                if self.using_gail:
                    ...
                    self.reward_giver = TransitionClassifier(self.observation_space, self.observation_space,
                                                             self.hidden_size_adversary,
                                                             entcoeff=self.adversary_entcoeff)
            ...
            with self.sess.as_default():
                ...
                while True:
                    ...
                    for k in range(self.g_step):
                        ...
                        observation, _, prev_observation = seg["observations"], seg["actions"], seg["prev_observations"]
                    ...
                    if self.using_gail:
                        ...
                        #                                               shuffle=True):
                        for ob_batch, prev_obs_batch in dataset.iterbatches((observation, prev_observation),
                                                                      include_final_partial_batch=False,
                                                                      batch_size=batch_size,
                                                                      shuffle=True):
                            ...
                            if isinstance(self.action_space, gym.spaces.Discrete):
                                if len(prev_obs_batch.shape) == 2:
                                    prev_obs_batch = prev_obs_batch[:, 0]
                                if len(prev_obs_expert.shape) == 2:
                                    prev_obs_expert = prev_obs_expert[:, 0]
                            *newlosses, grad = self.reward_giver.lossandgrad(ob_batch, prev_obs_batch, ob_expert, prev_obs_expert)
