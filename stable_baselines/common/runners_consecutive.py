...
def traj_segment_generator(policy, env, horizon, reward_giver=None, gail=False, callback=None):
    ...
    prev_observations = np.array([observation for _ in range(horizon)])
    ...
    while True:
        ...
        prev_observation = observations[i-1 if i > 0 else 0]
        prev_observations[i] = prev_observation
        vpreds[i] = vpred[0]
        actions[i] = action[0]
        ...
        if gail:
            reward = reward_giver.get_reward(observation, prev_observation)
